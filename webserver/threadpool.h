...
public:
	...
	void commit(F &&f, Args &&...args)
	{
		...
		auto task = std::make_shared<std::packaged_task<void()>>(
			std::bind(std::forward<F>(f), std::forward<Args>(args)...)); // std::forward<T>(u)有两个参数：T 与 u。当T为左值引用类型时，u将被转换为T类型的左值，否则u将被转换
...
private:
	...
	Task get_one_task()
	{
		...
		cv_task.wait(lock, [this]()
					 { return !tasks.empty() || m_exit; }); // wait直到有task
		if (m_exit)
		{					// 相当于std::function<void()>(nullptr);
